...
- [4. 解析と特殊プロット](#4-解析と特殊プロット)
  ...
  - [4-2. スペクトル解析と自動バックグラウンド除去 (remove_background)](#4-2-スペクトル解析と自動バックグラウンド除去-remove_background)
...
---
### 💡 この実装のポイント
バックグラウンド除去のアルゴリズムにおいて**「何を良いとするか（目的関数）」**の定義が難所ですが、今回のコードでは以下の3つのペナルティを合成して最適解を定義しています。
1. **Smoothness**: 抽出されたベースラインが滑らかであること（2階差分の分散最小化）。
2. **Non-negativity**: ピークが負の方向に沈み込まないこと。
3. **Sparsity**: ピークが存在しない領域のシグナルが、限りなくゼロに近づくこと。

これにより、ユーザーはあの難解な pybeads のパラメータを一切触ることなく、`auto_opt=True` の一言で美しいスペクトルを得られるようになります！
